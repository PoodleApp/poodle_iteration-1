...
function participants(activities: List<Zack>): { to: Address[], from: Address[], cc: Address[] } {
  var { to, from, cc } = activities.reduce((ls, [activity, message]) => ({
...
function flatParticipants(activities: List<Zack>): Address[] {
  var { from, to, cc } = participants(activities)
  return uniqBy(
    addr => addr.address,
