...
export type Verb = 'post' | 'share' | 'edit' | 'like' | 'unknown'
...
function contentPart(msg: Message): ?Attachment {
  if (textParts(msg).length > 0) {
    return textParts(msg)[0]
  }
  else if (htmlParts(msg).length > 0) {
    return htmlParts(msg)[0]
  }
}
...
function parseMidUri(uri: URI): ?{ msgId: MessageId, partId: ?string } {
  var parsed = parseMidOrCidUri(uri)
  if (parsed && parsed.scheme === 'mid:') {
    return {
      msgId: parsed.authority,
      partId: parsed.path,
    }
  }
}

function parseMidOrCidUri(uri: URI): ?{ scheme: string, authority: string, path: ?string } {
...
function resolveUri(msg: Message, uri: URI): URI {
  var parsed = parseMidOrCidUri(uri)
  if (parsed && parsed.scheme === 'cid:') {
    return `${midUri(msg)}/${parsed.authority}`
  }
  else {
    return uri
  }
