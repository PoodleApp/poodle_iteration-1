...
export class ComposeReply extends Sunshine.Component<{},ReplyProps,ReplyState> {
  ...
  render(): React.Element {
    ...
    return (
      <Paper>
        <ComposeOptsMenu style={styles.menu} />
        <form style={styles.body} onSubmit={this.onSend.bind(this)}>
  ...
  onSend(event: Event) {
    ...
    var addPeople = parseAddressList(this.refs.to.getValue()) || []
...
class ComposeOptsMenu extends Sunshine.Component<{},{},{}> {
  render() {
    return (
      <IconMenu
        iconButtonElement={
  ...
  onMenuAction(event: Event, item: React.Element) {
    if (item.props.value === 'addPeople') {
      this.emit(new CE.ShowAddPeople())
    }
  }
}
