...
import { participants, resolveUri } from '../activity'
...
export class ComposeReply extends Sunshine.Component<{},ReplyProps,ReplyState> {
  ...
  onSend(event: Event) {
    ...
    var [activity, message]     = last(conversation.activities)
    ...
    var author = { name: username, address: useremail }
    var note = Act.note({ verb: 'reply', author, subject, body, target: {
      objectType: 'activity',
      uri: resolveUri(message, activity.id),
    } })

    var draft: Draft = {
      activities: [note],
      from:       author,
      to:         withoutSelf(author, to),
      cc:         withoutSelf(author, without(to, cc)),
      subject,
